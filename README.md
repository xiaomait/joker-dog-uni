...
语录通过接口 `/api/message/dog` 获取。接口失败或未返回有效文本时，会自动切换到内置离线语录，保证页面始终可用。
...
业务接口定义位于 `request/index.js`，统一通过 `utils/request.js` 发起请求。首页调用 `/api/message/dog`，读取 `data.content` 与 `data.userName` 并展示；请求失败或数据为空时，通过 `utils/offline-quote.js` 获取一条本地语录兜底。
